...
public class WeatherSearchConsoleServiceImpl implements WeatherSearchService {
    ...
    public WeatherApiResponseDto requestCurrWeatherByLocation(double lat, double lon) {
        ...
        log.info("[WeatherSearchConsoleServiceImpl requestCurrWeatherByLocation] 날씨 조회 URI = {}", uri);
        WeatherApiResponseDto body = new WeatherApiResponseDto(new WeatherMainDto(273.13)); // kelvin
        convertTemperatureUnit(body);
        log.info("[WeatherSearchConsoleServiceImpl requestCurrWeatherByLocation] 날씨 조회 결과");
        log.info("[WeatherSearchConsoleServiceImpl requestCurrWeatherByLocation] body = {}", objectMapperService.serializeWeatherApiResponseDto(body));
        log.info("[WeatherSearchConsoleServiceImpl requestCurrWeatherByLocation] 온도 = {}", body.getMainDto().getTemp()); // 0도
        return body;
    }
